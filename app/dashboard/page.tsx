...
export default function Page() {
    ...
    const [currModel, setCurrModel] = useState<Model>({name: '', values: []});
        ...
        <>
            <div className="w-full bg-white rounded-lg shadow dark:bg-gray-800 p-4 md:p-6">
                <div className="flex justify-between mb-5">
                    <div>
                        <h5 className="leading-none text-3xl font-bold text-gray-900 dark:text-white pb-2">{currModel.name} Prediction</h5>
                ...
                <div id="predicted-chart">
                    <LineScatterChart title={currModel.name} data={Object.values(currModel.values)}/>
                </div>
                <div className="grid grid-cols-1 items-center border-gray-200 border-t dark:border-gray-700 justify-between mt-5">
                    <div className="flex justify-between items-center pt-5">
                        <Dropdown label={''} renderTrigger={() => <Button color={'gray'}>{currModel.name} <HiChevronDown className={'ml-2'}/> </Button>}>
                            {models.map((model: any, key: number) => (
                                <Dropdown.Item key={key} onClick={() => setCurrModel(model)}>
                                    {model.name}
                                </Dropdown.Item>
                            ))}
                        </Dropdown>
